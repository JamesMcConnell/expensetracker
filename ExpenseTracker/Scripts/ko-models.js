var PaycheckBudgetItem = function () {
	...
	self.updateStatus = function (item) {
		...
		app.dataservice.updatePaycheckBudgetItem(item.PaycheckBudgetItemId(), data);
	};
}
...
var PaycheckBudget = function () {
	...
	self.loadData = function (data) {
		...
		self.PaycheckBudgetItems(ko.utils.arrayMap(data.PaycheckBudgetItems, function (item) {
			var pbItem = new PaycheckBudgetItem();
			pbItem.loadData(item);
			return pbItem;
		}));
		...
		self.RemainingAmount(self.PaycheckBudgetAmount() - itemSum);
	};
